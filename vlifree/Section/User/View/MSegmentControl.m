...
- (id)initWithOriginY:(CGFloat)y Titles:(NSArray *)titles delegate:(id)delegate
{
    ...
    if (self = [super initWithFrame:rect4View]) {
        ...
        UIView * bottomLine = [[UIView alloc]initWithFrame:CGRectMake(0.0f, HYSegmentedControl_Height-1, _scrollView.width, 1.0f)];
        bottomLine.backgroundColor = [UIColor colorWithWhite:.8 alpha:1];
        ...
        _bottomLineView.backgroundColor = UIColorFromRGBValue(0xFF643D);
...
- (void)segmentedControlChange:(UIButton *)btn
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(mSegmentedControlSelectAtIndex:)]) {
        [self.delegate mSegmentedControlSelectAtIndex:btn.tag - 1000];
    }
}
...
{
    UIButton *btn = [_array4Btn objectAtIndex:0];
    [btn setTitle:[NSString stringWithFormat:@"外卖订单(%@)", orderCount] forState:UIControlStateNormal];
    
}
- (void)changeorderCount:(NSNumber *)orderCount
{
    UIButton *btn = [_array4Btn objectAtIndex:1];
    [btn setTitle:[NSString stringWithFormat:@"酒店订单(%@)", orderCount] forState:UIControlStateNormal];
    
}
- (void)changecollectCount:(NSNumber *)collectCount
{
    UIButton *btn = [_array4Btn objectAtIndex:2];
    [btn setTitle:[NSString stringWithFormat:@"我的收藏(%@)", collectCount] forState:UIControlStateNormal];
}
